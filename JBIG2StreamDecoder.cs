...
using System.IO;
using System.Linq;
using CondenseTech.JBig2Decoder.Decoders;
using CondenseTech.JBig2Decoder.Images;
using CondenseTech.JBig2Decoder.Segments.Extensions;
using CondenseTech.JBig2Decoder.Segments.Generic;
using CondenseTech.JBig2Decoder.Segments.Halftone;
using CondenseTech.JBig2Decoder.Segments.PageInformation;
using CondenseTech.JBig2Decoder.Segments.Pattern;
using CondenseTech.JBig2Decoder.Segments.Refinement;
using CondenseTech.JBig2Decoder.Segments.Region.text;
using CondenseTech.JBig2Decoder.Segments.Strips;
using CondenseTech.JBig2Decoder.Segments.SymbolDictionary;
using CondenseTech.JBig2Decoder.Utilities;
using SkiaSharp;
...
namespace CondenseTech.JBig2Decoder
{
    public class JBIG2StreamDecoder
    {
        ...
        private SKImage BitsToImage(byte[] input, int width, int height, int stride)
        {
            int n = 0;
                ...
                using (SKCanvas canvas = surface.Canvas)
                {
                    for (int y = 0; y < height; y++)
                    {
                        ...
                        for (int s = 0; s < stride; s++)
                        {
                            ...
                            {
                                SKPoint targetPoint = new SKPoint(x, y);
                                var bit = b & (1 << b0);
                                if (bit == 0)
                                {
                                    canvas.DrawPoint(targetPoint, SKColors.Black);
                                }
                                else
                                {
                                    canvas.DrawPoint(targetPoint, SKColors.White);
                                }
        ...
        /// <summary>
        /// This section is new for .net standard 2.0.  This is basically
        /// implementing WriteableBitmap since it is in .net core only.
        /// </summary>
        ...
        /// <returns></returns>
        private byte[] BitsToImageBuffer(byte[] input, int width, int height, int stride, SKEncodedImageFormat format)
        {
            int n = 0;
            SKImageInfo imageInfo = new SKImageInfo(width, height);
            using (SKSurface surface = SKSurface.Create(imageInfo))
        ...
        public byte[] DecodeJBIG2(byte[] data, SKEncodedImageFormat format = SKEncodedImageFormat.Png, int NewWidth = 0, int NewHeight = 0)
        {
            ...
            else
            {
                ...
                if (noOfPagesKnown)
                {
                    pageQuantity = GetNoOfPages();

                    if (JBIG2StreamDecoder.debug)
                        Console.WriteLine("pageQuantity = " + pageQuantity);
                }
            }
            ...
            var bmp = BitsToImageBuffer(dataStream, width, height, stride, format);
